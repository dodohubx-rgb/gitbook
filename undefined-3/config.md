...
| --------------------- | -------- | ------- | -------------------------------------------------------- | --------------------------------------------------------------------- |
| **global\_workspace** | no       | `true`  | ใช้ config ร่วมกันทุกบัญชี หรือแยกเป็นรายบัญชี           | Use one shared config for all accounts, or separate per account       |
| **auto\_hideui**      | no       | `false` | ซ่อน UI อัตโนมัติหลังจากเริ่มสคริปต์ หรือให้ UI แสดงอยู่ | Hide UI automatically after script initialization, or keep it visible |
